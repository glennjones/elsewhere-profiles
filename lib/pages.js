 ...
 * Pages object
 */
function Pages (objs, options, callback){
  this.pageCollection = [];
  this.completed = 0;
  this.urls = [];
  ...
  if(utils.isString(objs[0])){
    ...
    while (x < i) {
      ...
      this.pageCollection.push(new Page(this.urls[x], null, apiInterface, options))
      x++;
    }
  ...
  // turn array of identity objects into an array of page objects
    ...
    while (x < i) {
      ...
      if(url || apiInterface){
        this.pageCollection.push(new Page(url, identities[x], apiInterface, options))
      }
      x++;
    }

    // add metadata from mapper objects
    this.username = commonUserName(identities, this.options.logger);
    this.highestFrequencySite = highestFrequencySite(objs, false, this.options.logger);
    this.accounts = getAccountIdentities(identities, this.options.logger);
  }
  ...
  this.fetch(function(data){
    ...
    self.options.logger.info('total html request time: ' + totalRequestTime + 'ms');
    self.options.logger.info('total microformats parse time: ' + totalParseTime + 'ms');


    self.combinedProfile = combineProfiles(self.profiles, self.options.logger);
    ...
    self.options.logger.info('total time taken: ' 
...
function combineProfiles(profiles, logger){
  ...
  while (x < i) {
    ...
    var singleProps = ['name', 'family-name', 'additional-name', 'given-name', 'bday','class','geo','rev','role','sort-string','tz','uid'];
