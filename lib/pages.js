...
function Pages (objs, cache, logger, callback){
  ...
  if(utils.isString(objs[0])){
    ...
    var i = this.urls.length,
        x = 0;
    while (x < i) {
      this.pageCollection.push(new Page(this.urls[x], logger))
      x++;
    }
  ...
  // turn array of identity objects into an array of page objects
    ...
    while (x < i) {
      ...
      if(url){
        this.urls.push(url)
        this.pageCollection.push(new Page(url, logger, identities[x]))
      }
      x++;
    }
...
Pages.prototype = {
  ...
  fetch: function(cache, callback){
    ...
    // loop pages and fetch html then parse it
    while (x < i) {
      this.pageCollection[x].fetch(cache, pageHasParsed);
      x++;
    }
